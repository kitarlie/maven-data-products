...
**angle_distribution.py**: plots histograms of the IMF clock (angle away from the z-axis in the y-z plane) and cone (angle between the IMF and the Mars-Sun line) angles.
...
**omni_data_scraping**: takes stored OMNI data files (in .asc format) and summarises the magnitude of the IMF (magnetometer measurements when MAVEN is outside the bow shock) in frequency bins with resolution 0.1 nT, stored in CSVs.
